...
function createEntityTypes (projectId) {
  ...
  promises.push(conversationClient.createEntityType(sizeRequest)
      .then((responses) => {
  ...
  promises.push(conversationClient.createEntityType(toppingRequest)
      .then((responses) => {
...
function listEntityTypes (conversationClient, projectId) {
  ...
  return conversationClient.listEntityTypes(request)
      .then((responses) => {
...
function clearEntityTypes (projectId) {
  ...
  return listEntityTypes(conversationClient, projectId)
      .then((entityTypes) => {
        return Promise.all(
...
function deleteEntityType (conversationClient, entityType) {
  ...
  return conversationClient.deleteEntityType(request)
      .then((responses) => {
...
function getEntityType (conversationClient, entityType) {
  ...
  return conversationClient.getEntityType(request)
      .then((responses) => {
...
function updateEntityType (projectId, entityTypeId) {
  ...
  const entityTypePath =
      conversationClient.entityTypePath(projectId, agentId, entityTypeId);
  ...
  conversationClient.getEntityType(getEntityTypeRequest)
      .then((responses) => {
...
function createIntents (projectId) {
  ...
  const pizzaOutputContexts = [{
    name: conversationClient.contextPath(
        projectId, agentId, '*' /* sessionId */, 'pizza_order'),
    lifespanCount: 5
  }];
  ...
  conversationClient.createIntent(pizzaRequest)
      .then((responses) => {
  ...
  conversationClient.createIntent(changeDeliveryAddressRequest)
      .then((responses) => {
  ...
  conversationClient.createIntent(placeOrderRequest)
      .then((responses) => {
  ...
  conversationClient.createIntent(cancelOrderRequest)
      .then((responses) => {
...
function listIntents (conversationClient, projectId) {
  ...
  return conversationClient.listIntents(request)
      .then((responses) => {
...
function getIntent (conversationClient, intent) {
  ...
  return conversationClient.getIntent(request)
      .then((responses) => {
...
function deleteIntent (conversationClient, intent) {
  ...
  // Send the request for retrieving the intent.
  return conversationClient.deleteIntent(request)
      .then((responses) => {
...
function updateIntent (projectId, intentId) {
  ...
  const intentPath =
      conversationClient.intentPath(projectId, agentId, intentId);
  ...
  conversationClient.getIntent(getIntentRequest)
      .then((responses) => {
        ...
        if (intent.result.defaultResponsePlatforms
...
function logIntent (conversationClient, intent) {
  ...
  const outputContexts = intent.result.outputContexts.map((context) => {
  ...
  const defaultResponsePlatforms =
      intent.result.defaultResponsePlatforms.join(', ');
  console.log(
...
function createContext (projectId, sessionId) {
  ...
  conversationClient.createContext(pizzaContextRequest)
      .then((responses) => {
...
function listContexts (conversationClient, projectId, sessionId) {
  ...
  return conversationClient.listContexts(request)
      .then((responses) => {
...
function showContexts (projectId, sessionId) {
  ...
  return listContexts(conversationClient, projectId, sessionId)
      .then((contexts) => {
...
function getContext (conversationClient, context) {
  ...
  return conversationClient.getContext(request)
      .then((responses) => {
...
function deleteContext (conversationClient, context) {
  ...
  // Send the request for retrieving the context.
  return conversationClient.deleteContext(request)
      .then((responses) => {
...
function updateContext (projectId, sessionId, contextId) {
  ...
  const contextPath =
      conversationClient.contextPath(projectId, agentId, sessionId, contextId);
  ...
  conversationClient.getContext(getContextRequest)
      .then((responses) => {
...
function createSessionEntityType (projectId, sessionId) {
  ...
  const sizeSessionEntityTypePath = conversationClient.sessionEntityTypePath(
      projectId, agentId, sessionId, 'size');
  ...
  conversationClient.createSessionEntityType(sizeSessionEntityTypeRequest)
      .then((responses) => {
  ...
  const toppingSessionEntityTypePath = conversationClient.sessionEntityTypePath(
      projectId, agentId, sessionId, 'topping');
  ...
  conversationClient.createSessionEntityType(toppingSessionEntityTypeRequest)
      .then((responses) => {
...
function showSessionEntityTypes (projectId, sessionId) {
  ...
  // entity type names.
  listEntityTypes(conversationClient, projectId)
      .then((entityTypes) => {
        return Promise.all(entityTypes.map((entityType) => {
          return getSessionEntityType(
              conversationClient, projectId, sessionId, entityType.displayName);
        }));
      });
}

function getSessionEntityType (
    conversationClient, projectId, sessionId, entityTypeName) {
  // Agent ID must be set to project ID for v1alpha.
  const agentId = projectId;
  // The path to identify the sessionEntityType to be retrieved.
  const sessionEntityTypePath =
      conversationClient.sessionEntityTypePath(
          projectId, agentId, sessionId, entityTypeName);
  ...
  return conversationClient.getSessionEntityType(request)
      .then((responses) => {
...
function deleteSessionEntityType (
  ...
  // Send the request for retrieving the sessionEntityType.
  return conversationClient.deleteSessionEntityType(request)
      .then((responses) => {
...
function updateSessionEntityType (projectId, sessionId, entityTypeName) {
  ...
  conversationClient.updateSessionEntityType(request)
      .then((responses) => {
...
const cli = require(`yargs`)
    ...
    .command(
        ...
        {},
        (opts) => updateIntent(opts.projectId, opts.intentId))
    .command(
