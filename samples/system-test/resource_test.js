...
test.serial('Test creating / listing / deleting a context.', async t => {
  var output = await tools.runAsync(`${cmd} create-context -s ${sessionId} -c ${contextName} -l 3`);
  ...
  output = await tools.runAsync(`${cmd} delete-context -s ${sessionId} -c ${contextName}`);
  t.true(output.includes(sessionId));
  t.true(output.includes(contextName));
...
test.serial('Test creating / listing / deleting a entity type and entity.', async t => {
  // Create the Entity Type
  var output = await tools.runAsync(`${cmd} create-entity-type -d ${displayName} -k KIND_MAP`);
  ...
  output = await tools.runAsync(`${cmd} create-entity -e ${entityTypeId} -v ${entityName} -s ${synonym1} -s ${synonym2}`);
  ...
  // Delete the Entity
  output = await tools.runAsync(`${cmd} delete-entity -e ${entityTypeId} -v ${entityName}`);
  t.true(output.includes(entityName));
...
test.serial('Test creating / listing / deleting a intent.', async t => {
  var output = await tools.runAsync(`${cmd} create-intent -d ${displayName} -t ${phrase1} -t ${phrase2} -m ${message1} -m ${message2}`);
...
test.serial('Test creating / listing / deleting a session entity type', async t => {
  // Create the Entity Type
  var output = await tools.runAsync(`${cmd} create-entity-type -d ${displayName} -k KIND_MAP`);
  t.true(output.includes('entityTypes'));
  ...
  output = await tools.runAsync(`${cmd} create-session-entity-type -s ${sessionId} -e ${synonym1} -e ${synonym2} -d ${displayName} -o ENTITY_OVERRIDE_MODE_OVERRIDE`);
  t.true(output.includes(sessionId));
  t.true(output.includes(displayName));
  t.true(output.includes(synonym1));
  t.true(output.includes(synonym2));
  ...
  output = await tools.runAsync(`${cmd} list-session-entity-types -s ${sessionId}`);
  t.true(output.includes(sessionId));
  t.true(output.includes(displayName));
  t.true(output.includes('2'));

  // Delete the Session Entity Type
  output = await tools.runAsync(`${cmd} delete-session-entity-type -s ${sessionId} -d ${displayName}`);
  t.true(output.includes(displayName));
