...
describe('v2.ConversationProfilesClient', () => {
    ...
    describe('setSuggestionFeatureConfig', () => {
        it('invokes setSuggestionFeatureConfig without error', async () => {
            ...
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.setSuggestionFeatureConfig as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });
    ...
    describe('clearSuggestionFeatureConfig', () => {
        it('invokes clearSuggestionFeatureConfig without error', async () => {
            ...
            client.initialize();
            const request = generateSampleMessage(new protos.google.cloud.dialogflow.v2.ClearSuggestionFeatureConfigRequest());
            request.conversationProfile = '';
            const expectedHeaderRequestParams = "conversation_profile=";
            const expectedOptions = {
                otherArgs: {
            ...
            const [response] = await operation.promise();
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.clearSuggestionFeatureConfig as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });
